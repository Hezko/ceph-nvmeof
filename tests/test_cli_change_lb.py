...
def verify_namespaces_using_get_subsystems(
    caplog, gw_port, subsys, first_nsid, last_nsid, grp
):
    caplog.clear()
...
def verify_namespaces_using_spdk_get_subsystems(
    caplog, gw, subsys, first_nsid, last_nsid, grp
):
    caplog.clear()
...
def try_change_one_namespace_lb_group_no_listeners(
    caplog, subsys, nsid_to_change, new_group
):
    caplog.clear()
    cli(
        [
...
def change_one_namespace_lb_group(caplog, subsys, nsid_to_change, new_group):
    ...
    assert (
        f"Changing load balancing group of namespace {nsid_to_change} in {subsys} to {new_group}: Successful"
        in caplog.text
    )
    assert (
        f"Received request to change load balancing group for namespace with NSID {nsid_to_change} in {subsys} to {new_group}, context: <grpc._server"
        in caplog.text
    )
    ...
    assert (
        f"Received request to change load balancing group for namespace with NSID {nsid_to_change} in {subsys} to {new_group}, context: None"
        in caplog.text
    )
...
def test_change_namespace_lb_group(caplog, two_gateways):
    ...
    assert (
        f"Received request to add namespace to {subsystem}, ana group {anagrpid}, no_auto_visible: False, context: <grpc._server"
        in caplog.text
    )
    assert (
        f"Received request to add namespace 1 to {subsystem}, ana group {anagrpid}, no_auto_visible: False, context: None"
        in caplog.text
    )
    caplog.clear()
    cli(
    ...
    assert (
        f"Setting QOS limits of namespace 1 in {subsystem}: Successful" in caplog.text
    )
    caplog.clear()
    cli(
        [
    ...
    assert (
        f"Received request to add namespace to {subsystem}, ana group {anagrpid2}, no_auto_visible: False, context: <grpc._server"
        in caplog.text
    )
    assert (
        f"Received request to add namespace 2 to {subsystem}, ana group {anagrpid2}, no_auto_visible: False, context: None"
        in caplog.text
    )
    caplog.clear()
    cli(
    ...
    verify_namespaces_using_get_subsystems(
        caplog, "5501", subsystem, 1, namespace_count // 2, int(anagrpid)
    )
    verify_namespaces_using_get_subsystems(
        caplog,
        "5501",
    ...
    verify_namespaces_using_get_subsystems(
        caplog, "5502", subsystem, 1, namespace_count // 2, int(anagrpid)
    )
    verify_namespaces_using_get_subsystems(
        caplog,
        "5502",
    ...
    verify_namespaces_using_spdk_get_subsystems(
        caplog, gatewayA, subsystem, 1, namespace_count // 2, int(anagrpid)
    )
    verify_namespaces_using_spdk_get_subsystems(
        caplog,
        gatewayA,
    ...
    verify_namespaces_using_spdk_get_subsystems(
        caplog, gatewayB, subsystem, 1, namespace_count // 2, int(anagrpid)
    )
    verify_namespaces_using_spdk_get_subsystems(
        caplog,
        gatewayB,
    ...
    verify_namespaces(
        caplog, "5501", subsystem, 1 + (namespace_count // 2), namespace_count, anagrpid
    )
    verify_namespaces(caplog, "5502", subsystem, 1, namespace_count // 2, anagrpid2)
    verify_namespaces(
        caplog, "5502", subsystem, 1 + (namespace_count // 2), namespace_count, anagrpid
    )

    verify_namespaces_using_get_subsystems(
        caplog, "5501", subsystem, 1, namespace_count // 2, int(anagrpid2)
    )
    verify_namespaces_using_get_subsystems(
        caplog,
        "5501",
    ...
    verify_namespaces_using_get_subsystems(
        caplog, "5502", subsystem, 1, namespace_count // 2, int(anagrpid2)
    )
    verify_namespaces_using_get_subsystems(
        caplog,
        "5502",
    ...
    verify_namespaces_using_spdk_get_subsystems(
        caplog, gatewayA, subsystem, 1, namespace_count // 2, int(anagrpid2)
    )
    verify_namespaces_using_spdk_get_subsystems(
        caplog,
        gatewayA,
    ...
    verify_namespaces_using_spdk_get_subsystems(
        caplog, gatewayB, subsystem, 1, namespace_count // 2, int(anagrpid2)
    )
    verify_namespaces_using_spdk_get_subsystems(
        caplog,
        gatewayB,
