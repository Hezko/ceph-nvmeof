...
def test_create_get_subsys(caplog, config):
    ...
    with GatewayServer(config) as gateway:
        ...
        cli(
            [
                ...
                "add",
                "--subsystem",
                f"{subsystem_prefix}0",
                "--host-name",
                gateway.gateway_rpc.host_name,
                "--traddr",
                "127.0.0.1",
                "--trsvcid",
                "5001",
        ...
        assert (
            f"Adding {subsystem_prefix}0 listener at 127.0.0.1:5001: Successful"
            in caplog.text
        )
        ...
        assert (
            f"Setting QOS limits of namespace 1 in {subsystem_prefix}0: Successful"
            in caplog.text
        )
        assert (
            f"No previous QOS limits found, this is the first time the limits are set for namespace 1 on {subsystem_prefix}0"
            in caplog.text
        )
        caplog.clear()
        cli(
    ...
    with GatewayServer(config) as gateway:
        ...
        wait_for_string(
            caplog, f"Received request to create subsystem {subsystem_prefix}", 60
        )
        ...
        assert f"{subsystem_prefix}0 with ANA group id 1" in caplog.text
        assert (
            f"Received request to set QOS limits for namespace 1 on {subsystem_prefix}0, R/W IOs per second: 2000 Read megabytes per second: 5"
            in caplog.text
        )
        assert (
            f"Received request to set QOS limits for namespace 1 on {subsystem_prefix}0, R/W IOs per second: 2000 Read megabytes per second: 5"
        ...
        assert (
            f"Received request to add host {host_prefix}0 to namespace 1 on {subsystem_prefix}0, context: None"
            in caplog.text
        )
        caplog.clear()
        cli(["--format", "plain", "subsystem", "list"])
