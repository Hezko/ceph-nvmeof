...
def gateway(config):
    ...
    with GatewayServer(config) as gateway:
        ...
        ceph_utils.execute_ceph_monitor_command(
            "{"
            + f'"prefix":"nvme-gw create", "id": "{gateway.name}", "pool": "{pool}", "group": ""'
            + "}"
        )
        gateway.serve()
...
def test_create_secure(caplog, gateway):
    ...
    assert (
        f"Host {hostnqn4} has a DH-HMAC-CHAP key but subsystem {subsystem} has no key, a unidirectional authentication will be used"
        in caplog.text
    )
...
def test_create_secure_list(caplog, gateway):
    ...
    assert rc == 2
    assert (
        f"error: Can't have more than one host NQN when DH-HMAC-CHAP keys are used"
        in caplog.text
    )
...
def test_dhchap_controller_with_no_dhchap_key(caplog, gateway):
    ...
    assert (
        f"Failure adding host {hostnqn10} to {subsystem2}: Host must have a DH-HMAC-CHAP key if the subsystem has one"
        in caplog.text
    )
...
def test_add_key_to_host(caplog, gateway):
    ...
    assert (
        f"Changing key for host {hostnqn7} on subsystem {subsystem}: Successful"
        in caplog.text
    )
    assert (
        f"Host {hostnqn7} has a DH-HMAC-CHAP key but subsystem {subsystem} has no key, a unidirectional authentication will be used"
        in caplog.text
    )
    caplog.clear()
    found = False
...
def change_key_to_all_hosts(caplog, gateway):
    ...
    assert rc == 2
    assert (
        f"error: Can't change keys for host NQN '*', please use a real NQN"
        in caplog.text
    )
...
def change_key_for_host(caplog, gateway):
    ...
    assert (
        f"Changing keys for host {hostnqn7} on subsystem {subsystem}: Successful"
        in caplog.text
    )
    assert (
        f"Host {hostnqn7} has a DH-HMAC-CHAP key but subsystem {subsystem} has no key, a unidirectional authentication will be used"
...
def test_change_key_with_psk(caplog, gateway):
    ...
    assert (
        f"Changing key for host {hostnqn12} on subsystem {subsystem}: Successful"
        in caplog.text
    )
    assert (
        f"Host {hostnqn12} has a DH-HMAC-CHAP key but subsystem {subsystem} has no key, a unidirectional authentication will be used"
        in caplog.text
    )
...
def test_change_key_host_not_exist(caplog, gateway):
    ...
    assert (
        f"Failure changing DH-HMAC-CHAP key for host {hostnqn13} on subsystem {subsystem}: Can't find host on subsystem"
        in caplog.text
    )
...
def test_change_key_host_on_discovery(caplog, gateway):
    ...
    assert (
        f"Failure changing DH-HMAC-CHAP key for host {hostnqn12} on subsystem {discovery_nqn}: Can't use a discovery NQN as subsystem's"
        in caplog.text
    )
    caplog.clear()
    cli(
    ...
    assert (
        f"Failure changing DH-HMAC-CHAP key for host {discovery_nqn} on subsystem {subsystem}: Can't use a discovery NQN as host's"
        in caplog.text
    )
...
def test_change_key_host_invalid_nqn(caplog, gateway):
    ...
    assert (
        f'Failure changing DH-HMAC-CHAP key for host {hostnqn12} on subsystem {badsubsystem}: Invalid subsystem NQN "{badsubsystem}", contains invalid characters'
        in caplog.text
    )
    ...
    assert (
        f'Failure changing DH-HMAC-CHAP key for host {bad_hostnqn} on subsystem {subsystem}: Invalid host NQN "{bad_hostnqn}", contains invalid characters'
        in caplog.text
    )
    caplog.clear()
    cli(
    ...
    assert (
        f'Failure changing DH-HMAC-CHAP key for host {hostnqn12} on subsystem badnqn: NQN "badnqn" is too short, minimal length is 11'
        in caplog.text
    )
    caplog.clear()
    cli(
    ...
    assert (
        f'Failure changing DH-HMAC-CHAP key for host badnqn on subsystem {subsystem}: NQN "badnqn" is too short, minimal length is 11'
        in caplog.text
    )
...
def test_add_host_with_key_host_list(caplog, gateway):
    ...
    try:
        ...
        pass
    assert (
        "Can't have more than one host NQN when DH-HMAC-CHAP keys are used"
        in caplog.text
    )
    assert rc == 2
...
def test_set_subsystem_key_with_non_key_hosts(caplog, gateway):
    ...
    assert (
        f"Failure changing DH-HMAC-CHAP key for subsystem {subsystem}: Can't set a subsystem's DH-HMAC-CHAP key when it has hosts with no key, like host "
        in caplog.text
    )
...
def test_change_subsystem_key(caplog, gateway):
    ...
    assert (
        f"Received request to change inband authentication key for host {hostnqn11} on subsystem {subsystem2}"
        in caplog.text
    )
