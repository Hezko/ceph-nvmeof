...
def test_create_secure_with_any_host(caplog, gateway):
    ...
    assert (
        f'Secure channel is only allowed for subsystems in which "allow any host" is off'
        in caplog.text
    )
    caplog.clear()
...
def test_create_secure_list(caplog, gateway):
    ...
    assert rc == 2
    assert (
        f"error: Can't have more than one host NQN when PSK keys are used"
        in caplog.text
    )
...
def test_psk_with_dhchap(caplog, gateway):
    ...
    assert (
        f"Host {hostnqn10} has a DH-HMAC-CHAP key but subsystem {subsystem} has no key, a unidirectional authentication will be used"
        in caplog.text
    )
