    ...
    def remove_psk_host(self, subsys, host):
        if subsys in self.host_psk_key:
            ...
            if len(self.host_psk_key[subsys]) == 0:
                self.host_psk_key.pop(
                    subsys, None
                )  # last host was removed from subsystem
...
class NamespacesLocalList:
                ...
                if nsid in self.namespace_list[nqn]:
                    ...
                    if len(self.namespace_list[nqn]) == 0:
                        self.namespace_list.pop(
                            nqn, None
                        )  # last namespace of subsystem was removed
            else:
    ...
    def add_namespace(self, nqn, nsid, bdev, uuid, anagrpid, no_auto_visible):
        ...
        self.namespace_list[nqn][nsid] = NamespaceInfo(
            nsid, bdev, uuid, anagrpid, no_auto_visible
        )
        ...
        for one_subsys in subsystems:
            for nsid in self.namespace_list[one_subsys]:
                ...
                if no_auto_visible is not None:
                    if (
                        ns.no_auto_visible == no_auto_visible
                        and ns.host_count() >= min_hosts
                    ):
                        ns_count += 1
                else:
    ...
    def get_namespace_infos_for_anagrpid(
        self, nqn: str, anagrpid: int
    ) -> Iterator[NamespaceInfo]:
...
class GatewayService(pb2_grpc.GatewayServicer):
    ...
    def __init__(
        ...
        if not requested_hugepages_val:
            ...
            try:
                ...
                self.logger.info(
                    f"Requested huge pages count is {requested_hugepages_val}"
                )
            except ValueError:
                self.logger.warning(
                    f"Requested huge pages count value {requested_hugepages_val} is not numeric"
                )
        ...
        if not hugepages_file:
            ...
            self.logger.warning(
                "No huge pages file defined, will use /sys/kernel/mm/hugepages/hugepages-2048kB/nr_hugepages"
            )
        else:
            ...
            try:
                ...
                if hugepages_val:
                    try:
                        ...
                        self.logger.warning(
                            f"Actual huge pages count value {hugepages_val} is not numeric"
                        )
                        hugepages_val = ""
                    if (
                        requested_hugepages_val
                        and hugepages_val != ""
                        and requested_hugepages_val > hugepages_val
                    ):
                        self.logger.warning(
                            f"The actual huge page count {hugepages_val} is smaller than the requested value of {requested_hugepages_val}"
                        )
                else:
                    self.logger.warning(
                        f"Can't read actual huge pages count value from {hugepages_file}"
                    )
        ...
        override_hostname = self.config.get_with_default(
            "gateway", "override_hostname", ""
        )
        if override_hostname:
            self.host_name = override_hostname
            self.logger.info(
                f"Gateway's host name was overridden to {override_hostname}"
            )
        else:
            self.host_name = socket.gethostname()
        self.verify_nqns = self.config.getboolean_with_default(
            "gateway", "verify_nqns", True
        )
        ...
        self.max_hosts_per_namespace = self.config.getint_with_default(
            "gateway", "max_hosts_per_namespace", 1
        )
        self.max_namespaces_with_netmask = self.config.getint_with_default(
            "gateway", "max_namespaces_with_netmask", 1000
        )
        self.max_subsystems = self.config.getint_with_default(
            "gateway", "max_subsystems", GatewayService.MAX_SUBSYSTEMS_DEFAULT
        )
        self.max_namespaces = self.config.getint_with_default(
            "gateway", "max_namespaces", GatewayService.MAX_NAMESPACES_DEFAULT
        )
    ...
    def get_directories_for_key_file(
        self, key_type: str, subsysnqn: str, create_dir: bool = False
    ) -> []:
        tmp_dirs = []
    ...
    def create_host_key_file(
        self, key_type: str, subsysnqn: str, hostnqn: str, key_value: str
    ) -> str:
        ...
        tmp_dir_names = self.get_directories_for_key_file(
            key_type, subsysnqn, create_dir=True
        )
        ...
        try:
            (file_fd, filepath) = tempfile.mkstemp(
                prefix=keyfile_prefix, dir=tmp_dir_names[0], text=True
            )
        except Exception:
    ...
    def create_host_dhchap_file(
        self, subsysnqn: str, hostnqn: str, key_value: str
    ) -> str:
        return self.create_host_key_file(
            self.DHCHAP_PREFIX, subsysnqn, hostnqn, key_value
        )
    ...
    def remove_host_key_file(self, key_type: str, subsysnqn: str, hostnqn: str) -> None:
        ...
        tmp_dir_names = self.get_directories_for_key_file(
            key_type, subsysnqn, create_dir=False
        )
        if not tmp_dir_names:
            return
    ...
    @staticmethod
    def construct_key_name_for_keyring(
        subsysnqn: str, hostnqn: str, prefix: str = None
    ) -> str:
        key_name = (
            hashlib.sha256(subsysnqn.encode()).hexdigest()
            + "_"
            + hashlib.sha256(hostnqn.encode()).hexdigest()
        )
        if prefix:
    ...
    def remove_key_from_keyring(
        self, key_type: str, subsysnqn: str, hostnqn: str
    ) -> None:
        assert (
            self.rpc_lock.locked()
        ), "RPC is unlocked when calling remove_key_from_keyring()"
        key_name = GatewayService.construct_key_name_for_keyring(
            subsysnqn, hostnqn, key_type
        )
        try:
    ...
    def remove_dhchap_controller_key_from_keyring(
        self, subsysnqn: str, hostnqn: str
    ) -> None:
    ...
    def remove_all_subsystem_keys_from_keyring(self, subsysnqn: str) -> None:
        assert (
            self.rpc_lock.locked()
        ), "RPC is unlocked when calling remove_all_subsystem_keys_from_keyring()"
        try:
        ...
        for one_key in key_list:
            ...
            if key_path.startswith(
                f"{self.KEYS_DIR}/{self.PSK_PREFIX}_{subsysnqn}_"
            ) or key_path.startswith(
                f"{self.KEYS_DIR}/{self.DHCHAP_PREFIX}_{subsysnqn}_"
            ):
                try:
                    rpc_keyring.keyring_file_remove_key(self.spdk_rpc_client, key_name)
                except Exception:
    ...
    def _init_cluster_context(self) -> None:
        ...
        self.bdevs_per_cluster = self.config.getint_with_default(
            "spdk", "bdevs_per_cluster", 32
        )
        if self.bdevs_per_cluster < 1:
            raise Exception(
                f"invalid configuration: spdk.bdevs_per_cluster_contexts {self.bdevs_per_cluster} < 1"
            )
    ...
    def _get_cluster(self, anagrp: int) -> str:
        ...
        if not cluster_name:
            ...
            self.clusters[anagrp][cluster_name] += 1
        self.logger.info(
            f"get_cluster {cluster_name=} number bdevs: {self.clusters[anagrp][cluster_name]}"
        )
        return cluster_name
            ...
            if name in self.clusters[anagrp]:
                ...
                if self.clusters[anagrp][name] == 0:
                    ret = rpc_bdev.bdev_rbd_unregister_cluster(
                        self.spdk_rpc_client, name=name
                    )
                    ...
                    self.logger.info(
                        f"put_cluster {name=} number bdevs: {self.clusters[anagrp][name]}"
                    )
                return
    ...
    def _grpc_function_with_lock(self, func, request, context):
        with self.rpc_lock:
            ...
            if not self.omap_lock.omap_file_disable_unlock:
                assert (
                    not self.omap_lock.locked()
                ), f"OMAP is still locked when we're out of function {func}"
            return rc
    ...
    def create_bdev(
        ...
        if create_image:
            ...
            try:
                rc = self.ceph_utils.create_image(
                    rbd_pool_name, rbd_image_name, rbd_image_size
                )
                if rc:
                    self.logger.info(
                        f"Image {rbd_pool_name}/{rbd_image_name} created, size is {rbd_image_size} bytes"
                    )
                else:
                    self.logger.info(
                        f"Image {rbd_pool_name}/{rbd_image_name} already exists with size {rbd_image_size} bytes"
                    )
            except Exception as ex:
                errcode = 0
                msg = ""
                ...
                if not msg:
                    msg = str(ex)
                errmsg = (
                    f"Can't create RBD image {rbd_pool_name}/{rbd_image_name}: {msg}"
                )
        ...
        try:
            ...
            self.bdev_params[name] = {
                "uuid": uuid,
                "pool_name": rbd_pool_name,
                "image_name": rbd_image_name,
                "image_size": rbd_image_size,
                "block_size": block_size,
            }

            self.logger.debug(
                f"bdev_rbd_create: {bdev_name}, cluster_name {cluster_name}"
            )
        except Exception as ex:
        ...
        assert (
            name == bdev_name
        ), f"Created bdev name {bdev_name} differs from requested name {name}"
    ...
    def resize_bdev(self, bdev_name, new_size, peer_msg=""):
        ...
        self.logger.info(
            f"Received request to resize bdev {bdev_name} to {new_size} MiB{peer_msg}"
        )
        ...
        if bdev_info is not None:
            try:
                ...
                self.logger.warning(
                    f"Key {err} is not found, will not check size for shrinkage"
                )
                pass
        else:
            self.logger.warning(
                f"Can't get information for associated block device {bdev_name}, won't check size for shrinkage"
            )

        if rbd_pool_name and rbd_image_name:
            try:
                current_size = self.ceph_utils.get_image_size(
                    rbd_pool_name, rbd_image_name
                )
                ...
                self.logger.warning(
                    f"Error trying to get the size of image {rbd_pool_name}/{rbd_image_name}, won't check size for shrinkage:\n{ex}"
                )
                pass
    ...
    def subsystem_already_exists(self, context, nqn) -> bool:
        ...
        for key, val in state.items():
            ...
            try:
                ...
                self.logger.exception(
                    f"Got exception while parsing {val}, will continue"
                )
                continue
        return False
    ...
    def create_subsystem_safe(self, request, context):
        ...
        create_subsystem_error_prefix = (
            f"Failure creating subsystem {request.subsystem_nqn}"
        )
        ...
        if not request.enable_ha:
            errmsg = (
                f"{create_subsystem_error_prefix}: HA must be enabled for subsystems"
            )
            self.logger.error(f"{errmsg}")
            return pb2.subsys_status(
                status=errno.EINVAL, error_message=errmsg, nqn=request.subsystem_nqn
            )
        ...
        if not request.max_namespaces:
            ...
            if request.max_namespaces > self.max_namespaces:
                self.logger.warning(
                    f"The requested max number of namespaces for subsystem {request.subsystem_nqn} ({request.max_namespaces}) is greater than the global limit on the number of namespaces ({self.max_namespaces}), will continue"
                )
        ...
        if self.verify_nqns:
            ...
            if rc[0] != 0:
                ...
                self.logger.error(f"{errmsg}")
                return pb2.subsys_status(
                    status=rc[0], error_message=errmsg, nqn=request.subsystem_nqn
                )

        if GatewayUtils.is_discovery_nqn(request.subsystem_nqn):
            errmsg = (
                f"{create_subsystem_error_prefix}: Can't create a discovery subsystem"
            )
            self.logger.error(f"{errmsg}")
            return pb2.subsys_status(
                status=errno.EINVAL, error_message=errmsg, nqn=request.subsystem_nqn
        ...
        if len(self.subsys_max_ns) >= self.max_subsystems:
            ...
            self.logger.error(f"{errmsg}")
            return pb2.subsys_status(
                status=errno.E2BIG, error_message=errmsg, nqn=request.subsystem_nqn
            )
        ...
        if context:
            if request.no_group_append or not self.gateway_group:
                ...
                self.logger.info(
                    f"Subsystem NQN was changed to {request.subsystem_nqn}, adding the group name"
                )
        ...
        if not request.serial_number:
            ...
            self.logger.info(
                f"No serial number specified for {request.subsystem_nqn}, will use {request.serial_number}"
            )
        ...
        with omap_lock:
            ...
            try:
                ...
                subsys_already_exists = self.subsystem_already_exists(
                    context, request.subsystem_nqn
                )
                if subsys_already_exists:
                    errmsg = f"Subsystem already exists"
                else:
                    subsys_using_serial = self.serial_number_already_used(
                        context, request.serial_number
                    )
                ...
                if request.dhchap_key:
                    self.host_info.add_dhchap_key_to_subsystem(
                        request.subsystem_nqn, request.dhchap_key
                    )
                ...
                return pb2.subsys_status(
                    status=status, error_message=errmsg, nqn=request.subsystem_nqn
                )
        ...
        return pb2.subsys_status(
            status=0, error_message=os.strerror(0), nqn=request.subsystem_nqn
        )
    ...
    def get_subsystem_namespaces(self, nqn) -> list:
        ...
        for key, val in local_state_dict.items():
            ...
            try:
                ...
                self.logger.exception(
                    f"Got exception trying to get subsystem {nqn} namespaces"
                )
                pass
    ...
    def subsystem_has_listeners(self, nqn) -> bool:
        ...
        for key, val in local_state_dict.items():
            ...
            try:
                ...
                self.logger.exception(
                    f"Got exception trying to get subsystem {nqn} listener"
                )
                pass
    ...
    def delete_subsystem_safe(self, request, context):
        ...
        delete_subsystem_error_prefix = (
            f"Failure deleting subsystem {request.subsystem_nqn}"
        )
        ...
        with omap_lock:
            try:
                ...
                self.subsystem_nsid_bdev_and_uuid.remove_namespace(
            ...
            if not ret:
                ...
                self.remove_subsystem_from_state(request.subsystem_nqn, context)
                return pb2.req_status(
                    status=errno.EINVAL, error_message=delete_subsystem_error_prefix
                )
    ...
    def delete_subsystem(self, request, context=None):
        ...
        self.logger.info(
            f"Received request to delete subsystem {request.subsystem_nqn}, context: {context}{peer_msg}"
        )
        ...
        if GatewayUtils.is_discovery_nqn(request.subsystem_nqn):
            errmsg = (
                f"{delete_subsystem_error_prefix}: Can't delete a discovery subsystem"
            )
            self.logger.error(f"{errmsg}")
            return pb2.req_status(status=errno.EINVAL, error_message=errmsg)
        ...
        if context:
            if self.subsystem_has_listeners(request.subsystem_nqn):
                self.logger.warning(
                    f"About to delete subsystem {request.subsystem_nqn} which has a listener defined"
                )
        ...
        for nsid in ns_list:
            ...
            self.logger.warning(
                f"Will remove namespace {nsid} from {request.subsystem_nqn}"
            )
            ret = self.namespace_delete(
                pb2.namespace_delete_req(
                    subsystem_nqn=request.subsystem_nqn, nsid=nsid
                ),
                context,
            )
            if ret.status == 0:
                self.logger.info(
                    f"Automatically removed namespace {nsid} from {request.subsystem_nqn}"
                )
            else:
                self.logger.error(
                    f"Failure removing namespace {nsid} from {request.subsystem_nqn}:\n{ret.error_message}"
                )
                self.logger.warning(
                    f"Will continue deleting {request.subsystem_nqn} anyway"
                )
    ...
    def check_if_image_used(self, pool_name, image_name):
        ...
        for key, val in state.items():
            ...
            try:
                ...
                if (
                    pool_name
                    and pool_name == ns_pool
                    and image_name
                    and image_name == ns_image
                ):
    ...
    def create_namespace(
        self, subsystem_nqn, bdev_name, nsid, anagrpid, uuid, no_auto_visible, context
    ):
        """Adds a namespace to a subsystem."""

        if context:
            assert (
                self.omap_lock.locked()
            ), "OMAP is unlocked when calling create_namespace()"
        ...
        add_namespace_error_prefix = (
            f"Failure adding namespace{nsid_msg} to {subsystem_nqn}"
        )

        peer_msg = self.get_peer_message(context)
        self.logger.info(
            f"Received request to add {bdev_name} to {subsystem_nqn} with ANA group id {anagrpid}{nsid_msg}, no_auto_visible: {no_auto_visible}, context: {context}{peer_msg}"
        )
        ...
        if (
            no_auto_visible
            and self.subsystem_nsid_bdev_and_uuid.get_namespace_count(
                subsystem_nqn, True, 0
            )
            >= self.max_namespaces_with_netmask
        ):
        ...
        if (
            not nsid
            and self.subsystem_nsid_bdev_and_uuid.get_namespace_count(
                subsystem_nqn, None, 0
            )
            >= self.subsys_max_ns[subsystem_nqn]
        ):
        ...
        if (
            self.subsystem_nsid_bdev_and_uuid.get_namespace_count(None, None, 0)
            >= self.max_namespaces
        ):
        ...
        try:
            ...
            self.subsystem_nsid_bdev_and_uuid.add_namespace(
                subsystem_nqn, nsid, bdev_name, uuid, anagrpid, no_auto_visible
            )
        ...
        if not nsid:
            ...
            return pb2.nsid_status(
                status=errno.EINVAL, error_message=add_namespace_error_prefix
            )
    ...
    def set_ana_state_safe(self, ana_info: pb2.ana_info, context=None):
        ...
        self.logger.info(
            f"Received request to set ana states {ana_info.states}, {peer_msg}"
        )
                ...
                for gs in nas.states:
                    ...
                    try:
                        ...
                        if gs.state == pb2.ana_state.OPTIMIZED:
                            ...
                            for (
                                ns_info
                            ) in self.subsystem_nsid_bdev_and_uuid.get_namespace_infos_for_anagrpid(
                                nqn, grp_id
                            ):
                                ...
                                if not cluster:
                                    raise Exception(
                                        f"can not find cluster context name for bdev {ns_info.bdev}"
                                    )
                                ...
                                if not rpc_bdev.bdev_rbd_wait_for_latest_osdmap(
                                    ...
                                    raise Exception(
                                        f"bdev_rbd_wait_for_latest_osdmap({cluster=}) error"
                                    )
                                self.logger.debug(
                                    f"set_ana_state bdev_rbd_wait_for_latest_osdmap {cluster=}"
                                )
                                awaited_cluster_contexts.add(cluster)

                        self.logger.debug(
                            f"set_ana_state nvmf_subsystem_listener_set_ana_state {nqn=} {listener=} {ana_state=} {grp_id=}"
                        )
                        ...
                        self.logger.debug(
                            f"set_ana_state nvmf_subsystem_listener_set_ana_state response {ret=}"
                        )
                        if not ret:
                            raise Exception(
                                f"nvmf_subsystem_listener_set_ana_state({nqn=}, {listener=}, {ana_state=}, {grp_id=}) error"
                            )
                    except Exception as ex:
    ...
    def choose_anagrpid_for_namespace(self, nsid) -> int:
        ...
        for ana_grp in grps_list:
            if not self.clusters[
                ...
                self.logger.info(
                    f"New GW created: chosen ana group {ana_grp} for ns {nsid} "
                )
                return ana_grp
        ...
        for ana_grp in self.clusters:
            if ana_grp in grps_list:  # to take into consideration only valid groups
                ...
                for name in self.clusters[ana_grp]:
                    ana_load[ana_grp] += self.clusters[ana_grp][
                        name
                    ]  # accumulate the total load per ana group for all valid ana_grp clusters
        for ana_grp in ana_load:
            ...
            if ana_load[ana_grp] <= min_load:
                ...
                self.logger.info(
                    f" ana group {ana_grp} load =  {ana_load[ana_grp]} set as min {min_load} "
                )
        self.logger.info(
            f"Found min loaded cluster: chosen ana group {chosen_ana_group} for ns {nsid} "
        )
    ...
    def namespace_add_safe(self, request, context):
        ...
        self.logger.info(
            f"Received request to add namespace {nsid_msg}to {request.subsystem_nqn}, ana group {request.anagrpid}, no_auto_visible: {request.no_auto_visible}, context: {context}{peer_msg}"
        )
        ...
        if context:
            ...
            if request.nsid:
                ns = self.subsystem_nsid_bdev_and_uuid.find_namespace(
            ...
            ns = self.subsystem_nsid_bdev_and_uuid.find_namespace(
                request.subsystem_nqn, None, request.uuid
            )
            if not ns.empty():
                errmsg = (
                    f"Failure adding namespace, UUID {request.uuid} is already in use"
                )
                self.logger.error(f"{errmsg}")
                return pb2.nsid_status(status=errno.EEXIST, error_message=errmsg)
        ...
        with omap_lock:
            if context:
                errmsg, ns_nqn = self.check_if_image_used(
                    request.rbd_pool_name, request.rbd_image_name
                )
                if errmsg and ns_nqn:
                    if request.force:
                        self.logger.warning(
                            f'{errmsg}, will continue as the "force" argument was used'
                        )
                    else:
                        ...
                        self.logger.error(errmsg)
                        return pb2.nsid_status(
                            status=errno.EEXIST, error_message=errmsg
                        )
                ...
                if ret_bdev.status != errno.EEXIST:
                    ...
                    if ns_bdev != None:
                        try:
                            ...
                            self.logger.debug(
                                f"delete_bdev({bdev_name}): {ret_del.status}"
                            )
            ...
            ret_ns = self.create_namespace(
                request.subsystem_nqn,
                bdev_name,
                request.nsid,
                anagrp,
                request.uuid,
                request.no_auto_visible,
                context,
            )
            ...
            if ret_ns.status != 0:
                try:
                    ...
                    if ret_del.status != 0:
                        self.logger.warning(
                            f"Failure {ret_del.status} deleting bdev {bdev_name}: {ret_del.error_message}"
                        )
                except AssertionError:
                    self.logger.exception(
                        f"Got an assert while trying to delete bdev {bdev_name}"
            ...
            if context:
                ...
                try:
                    ...
                    self.gateway_state.add_namespace(
                        request.subsystem_nqn, ret_ns.nsid, json_req
                    )
                except Exception as ex:
    ...
    def namespace_change_load_balancing_group_safe(self, request, context):
        ...
        self.logger.info(
            f"Received request to change load balancing group for namespace with NSID {request.nsid} in {request.subsystem_nqn} to {request.anagrpid}, context: {context}{peer_msg}"
        )
        ...
        with omap_lock:
            ...
            if context:
                ...
                state = self.gateway_state.local.get_state()
                ns_key = GatewayState.build_namespace_key(
                    request.subsystem_nqn, request.nsid
                )
                try:
                ...
                self.logger.debug(
                    f"ANA group of ns#{request.nsid} - {anagrp} is owned by gateway {gw_id}, self.name is {self.gateway_name}"
                )
            ...
            if not ret:
                ...
                return pb2.req_status(
                    status=errno.EINVAL, error_message=change_lb_group_failure_prefix
                )
            ...
            if context:
                ...
                # Update gateway state
                try:
                    add_req = pb2.namespace_add_req(
                        rbd_pool_name=ns_entry["rbd_pool_name"],
                    ...
                    self.gateway_state.add_namespace(
                        request.subsystem_nqn, request.nsid, json_req
                    )
                except Exception as ex:
    ...
    def namespace_change_load_balancing_group(self, request, context=None):
        ...
        return self.execute_grpc_function(
            self.namespace_change_load_balancing_group_safe, request, context
        )
    ...
    def remove_namespace_from_state(self, nqn, nsid, context):
        ...
        assert (
            self.omap_lock.locked()
        ), "OMAP is unlocked when calling remove_namespace_from_state()"
    ...
    def remove_namespace(self, subsystem_nqn, nsid, context):
        ...
        if context:
            assert (
                self.omap_lock.locked()
            ), "OMAP is unlocked when calling remove_namespace()"
        peer_msg = self.get_peer_message(context)
        namespace_failure_prefix = (
            f"Failure removing namespace {nsid} from {subsystem_nqn}"
        )
        self.logger.info(
            f"Received request to remove namespace {nsid} from {subsystem_nqn}{peer_msg}"
        )
        ...
        if not ret:
            ...
            return pb2.req_status(
                status=errno.EINVAL, error_message=namespace_failure_prefix
            )
    ...
    def get_bdev_info(self, bdev_name):
        ...
        try:
            ...
            if len(bdevs) > 1:
                self.logger.warning(
                    f"Got {len(bdevs)} bdevs for bdev name {bdev_name}, will use the first one"
                )
            ret_bdev = bdevs[0]
        except Exception:
    ...
    def list_namespaces(self, request, context=None):
        ...
        self.logger.info(
            f"Received request to list {nsid_msg} for {request.subsystem}, context: {context}{peer_msg}"
        )

        if not request.subsystem:
            errmsg = f"Failure listing namespaces, missing subsystem NQN"
            self.logger.error(f"{errmsg}")
            return pb2.namespaces_info(
                status=errno.EINVAL,
                error_message=errmsg,
        ...
        with self.rpc_lock:
            try:
                ret = rpc_nvmf.nvmf_get_subsystems(
                    self.spdk_rpc_client, nqn=request.subsystem
                )
        ...
        for s in ret:
            try:
                if s["nqn"] != request.subsystem:
                    self.logger.warning(
                        f'Got subsystem {s["nqn"]} instead of {request.subsystem}, ignore'
                    )
                    continue
                try:
                ...
                for n in ns_list:
                    ...
                    if request.nsid and request.nsid != n["nsid"]:
                        self.logger.debug(
                            f'Filter out namespace {n["nsid"]} which is different than requested nsid {request.nsid}'
                        )
                        continue
                    if request.uuid and request.uuid != n["uuid"]:
                        self.logger.debug(
                            f'Filter out namespace with UUID {n["uuid"]} which is different than requested UUID {request.uuid}'
                        )
                        continue
                    lb_group = 0
                    try:
                        ...
                        pass
                    find_ret = self.subsystem_nsid_bdev_and_uuid.find_namespace(
                        request.subsystem, nsid
                    )
                    if find_ret.empty():
                        self.logger.warning(
                            f"Can't find info of namesapce {nsid} in {request.subsystem}. Visibility status will be inaccurate"
                        )
                    no_auto_visible = find_ret.no_auto_visible
                    one_ns = pb2.namespace_cli(
                        nsid=nsid,
                        bdev_name=bdev_name,
                        uuid=n["uuid"],
                    ...
                    if ns_bdev == None:
                        self.logger.warning(
                            f"Can't find namespace's bdev {bdev_name}, will not list bdev's information"
                        )
                    else:
                        try:
                            ...
                            one_ns.rbd_image_size = (
                                ns_bdev["block_size"] * ns_bdev["num_blocks"]
                            )
                            assigned_limits = ns_bdev["assigned_rate_limits"]
                            one_ns.rw_ios_per_second = assigned_limits["rw_ios_per_sec"]
                            one_ns.rw_mbytes_per_second = assigned_limits[
                                "rw_mbytes_per_sec"
                            ]
                            one_ns.r_mbytes_per_second = assigned_limits[
                                "r_mbytes_per_sec"
                            ]
                            one_ns.w_mbytes_per_second = assigned_limits[
                                "w_mbytes_per_sec"
                            ]
                        except KeyError as err:
                            self.logger.warning(
                                f"Key {err} is not found, will not list bdev's information"
                            )
                            pass
                        except Exception:
    ...
    def namespace_get_io_stats(self, request, context=None):
        ...
        peer_msg = self.get_peer_message(context)
        self.logger.info(
            f"Received request to get IO stats for namespace {request.nsid} on {request.subsystem_nqn}, context: {context}{peer_msg}"
        )
        if not request.nsid:
        ...
        with self.rpc_lock:
            ...
            if find_ret.empty():
                ...
                self.logger.error(errmsg)
                return pb2.namespace_io_stats_info(
                    status=errno.ENODEV, error_message=errmsg
                )
            uuid = find_ret.uuid
        ...
        try:
            ...
            if len(bdevs) > 1:
                self.logger.warning(
                    f"More than one associated block device found for namespace, will use the first one"
                )
            bdev = bdevs[0]
            io_errs = []
            try:
                ...
                for err_name in io_error.keys():
                    one_error = pb2.namespace_io_error(
                        name=err_name, value=io_error[err_name]
                    )
    ...
    def get_qos_limits_string(self, request):
        ...
        if request.HasField("rw_mbytes_per_second"):
            limits_to_set += (
                f" R/W megabytes per second: {request.rw_mbytes_per_second}"
            )
        if request.HasField("r_mbytes_per_second"):
            limits_to_set += (
                f" Read megabytes per second: {request.r_mbytes_per_second}"
            )
        if request.HasField("w_mbytes_per_second"):
            limits_to_set += (
                f" Write megabytes per second: {request.w_mbytes_per_second}"
            )
    ...
    def namespace_set_qos_limits_safe(self, request, context):
        ...
        self.logger.info(
            f"Received request to set QOS limits for namespace {request.nsid} on {request.subsystem_nqn},{limits_to_set}, context: {context}{peer_msg}"
        )
        ...
        if context:
            state = self.gateway_state.local.get_state()
            ns_qos_key = GatewayState.build_namespace_qos_key(
                request.subsystem_nqn, request.nsid
            )
            try:
                ...
                self.logger.info(
                    f"No previous QOS limits found, this is the first time the limits are set for namespace {request.nsid} on {request.subsystem_nqn}"
                )

        # Merge current limits with previous ones, if exist
        if ns_qos_entry:
            if (
                not request.HasField("rw_ios_per_second")
                and ns_qos_entry.get("rw_ios_per_second") != None
            ):
                request.rw_ios_per_second = int(ns_qos_entry["rw_ios_per_second"])
            if (
                not request.HasField("rw_mbytes_per_second")
                and ns_qos_entry.get("rw_mbytes_per_second") != None
            ):
                request.rw_mbytes_per_second = int(ns_qos_entry["rw_mbytes_per_second"])
            if (
                not request.HasField("r_mbytes_per_second")
                and ns_qos_entry.get("r_mbytes_per_second") != None
            ):
                request.r_mbytes_per_second = int(ns_qos_entry["r_mbytes_per_second"])
            if (
                not request.HasField("w_mbytes_per_second")
                and ns_qos_entry.get("w_mbytes_per_second") != None
            ):
                request.w_mbytes_per_second = int(ns_qos_entry["w_mbytes_per_second"])

            limits_to_set = self.get_qos_limits_string(request)
            self.logger.debug(
                f"After merging current QOS limits with previous ones for namespace {request.nsid} on {request.subsystem_nqn},{limits_to_set}"
            )
        ...
        with omap_lock:
            try:
                ret = rpc_bdev.bdev_set_qos_limit(
                    self.spdk_rpc_client, **set_qos_limits_args
                )
                ...
                if resp:
                    status = resp["code"]
                    errmsg = (
                        f"Failure setting namespace's QOS limits: {resp['message']}"
                    )
                return pb2.req_status(status=status, error_message=errmsg)
            ...
            if context:
                ...
                try:
                    ...
                    self.gateway_state.add_namespace_qos(
                        request.subsystem_nqn, request.nsid, json_req
                    )
                except Exception as ex:
    ...
    def namespace_set_qos_limits(self, request, context=None):
        """Set namespace's qos limits."""
        return self.execute_grpc_function(
            self.namespace_set_qos_limits_safe, request, context
        )
    ...
    def namespace_resize_safe(self, request, context=None):
        ...
        peer_msg = self.get_peer_message(context)
        self.logger.info(
            f"Received request to resize namespace {request.nsid} on {request.subsystem_nqn} to {request.new_size} MiB, context: {context}{peer_msg}"
        )
        ...
        if request.new_size <= 0:
            errmsg = (
                f"Failure resizing namespace {request.nsid}: New size must be positive"
            )
            self.logger.error(f"{errmsg}")
            return pb2.req_status(status=errno.EINVAL, error_message=errmsg)
    ...
    def namespace_delete_safe(self, request, context):
        ...
        peer_msg = self.get_peer_message(context)
        self.logger.info(
            f"Received request to delete namespace {request.nsid} from {request.subsystem_nqn}, context: {context}{peer_msg}"
        )
        ...
        if not bdev_name:
            self.logger.warning(
                f"Can't find namespace's bdev name, will try to delete namespace anyway"
            )
        ...
        with omap_lock:
            ...
            self.remove_namespace_from_state(
                request.subsystem_nqn, request.nsid, context
            )
            self.subsystem_nsid_bdev_and_uuid.remove_namespace(
                request.subsystem_nqn, request.nsid
            )
    ...
    def namespace_add_host_safe(self, request, context):
        ...
        peer_msg = self.get_peer_message(context)
        self.logger.info(
            f"Received request to add host {request.host_nqn} to namespace {request.nsid} on {request.subsystem_nqn}, context: {context}{peer_msg}"
        )
        ...
        with omap_lock:
            ...
            if context:
                ...
                try:
                    ...
                    self.gateway_state.add_namespace_host(
                        request.subsystem_nqn, request.nsid, request.host_nqn, json_req
                    )
                except Exception as ex:
    ...
    def namespace_add_host(self, request, context=None):
        """Add a host to a namespace."""
        return self.execute_grpc_function(
            self.namespace_add_host_safe, request, context
        )
    ...
    def namespace_delete_host_safe(self, request, context):
        ...
        peer_msg = self.get_peer_message(context)
        self.logger.info(
            f"Received request to delete host {request.host_nqn} from namespace {request.nsid} on {request.subsystem_nqn}, context: {context}{peer_msg}"
        )
        ...
        with omap_lock:
            ...
            if context:
                # Update gateway state
                try:
                    self.gateway_state.remove_namespace_host(
                        request.subsystem_nqn, request.nsid, request.host_nqn
                    )
                except Exception as ex:
    ...
    def namespace_delete_host(self, request, context=None):
        """Delete a host from a namespace."""
        return self.execute_grpc_function(
            self.namespace_delete_host_safe, request, context
        )
    ...
    def _create_dhchap_key_files(
        self, subsystem_nqn, host_nqn, dhchap_key, dhchap_ctrlr_key, err_prefix
    ):
        assert dhchap_key, "DH-HMAC-CHAP key value can't be empty"
        dhchap_file = None
        dhchap_key_name = None
        if dhchap_key:
            dhchap_file = self.create_host_dhchap_file(
                subsystem_nqn, host_nqn, dhchap_key
            )
            if not dhchap_file:
        ...
        if dhchap_ctrlr_key:
            dhchap_ctrlr_file = self.create_host_dhchap_file(
                subsystem_nqn, host_nqn, dhchap_ctrlr_key
            )
    ...
    def _add_key_to_keyring(self, keytype, filename, keyname):
        ...
        try:
            ret = rpc_keyring.keyring_file_add_key(
                self.spdk_rpc_client, keyname, filename
            )
            self.logger.debug(
                f"keyring_file_add_key {keyname} and file {filename}: {ret}"
            )
    ...
    def add_host_safe(self, request, context):
        ...
        if request.host_nqn == "*":
            self.logger.info(
                f"Received request to allow any host access for {request.subsystem_nqn}, context: {context}{peer_msg}"
            )
        else:
            self.logger.info(
        ...
        all_host_failure_prefix = (
            f"Failure allowing open host access to {request.subsystem_nqn}"
        )
        host_failure_prefix = (
            f"Failure adding host {request.host_nqn} to {request.subsystem_nqn}"
        )
        ...
        if request.host_nqn == "*" and self.host_info.does_subsystem_have_dhchap_key(
            request.subsystem_nqn
        ):
        ...
        if GatewayUtils.is_discovery_nqn(request.subsystem_nqn):
            if request.host_nqn == "*":
                ...
                errmsg = (
                    f"{host_failure_prefix}: Can't add host to a discovery subsystem"
                )
            self.logger.error(f"{errmsg}")
            return pb2.req_status(status=errno.EINVAL, error_message=errmsg)
        ...
        if request.psk and request.host_nqn == "*":
            errmsg = (
                f"{all_host_failure_prefix}: PSK is only allowed for specific hosts"
            )
            self.logger.error(f"{errmsg}")
            return pb2.req_status(status=errno.EINVAL, error_message=errmsg)
        ...
        if request.dhchap_key and not self.host_info.does_subsystem_have_dhchap_key(
            request.subsystem_nqn
        ):
            self.logger.warning(
                f"Host {request.host_nqn} has a DH-HMAC-CHAP key but subsystem {request.subsystem_nqn} has no key, a unidirectional authentication will be used"
            )
        ...
        host_already_exist = self.matching_host_exists(
            context, request.subsystem_nqn, request.host_nqn
        )
        if host_already_exist:
            if request.host_nqn == "*":
                errmsg = (
                    f"{all_host_failure_prefix}: Open host access is already allowed"
                )
                self.logger.error(f"{errmsg}")
        ...
        if (
            request.host_nqn != "*"
            and self.host_info.get_host_count(request.subsystem_nqn)
            >= self.max_hosts_per_subsystem
        ):
        ...
        dhchap_ctrlr_key = self.host_info.get_subsystem_dhchap_key(
            request.subsystem_nqn
        )
        if dhchap_ctrlr_key:
            self.logger.info(
                f"Got DHCHAP key {dhchap_ctrlr_key} for subsystem {request.subsystem_nqn}"
            )
        ...
        psk_key_name = None
        if request.psk:
            psk_file = self.create_host_psk_file(
                request.subsystem_nqn, request.host_nqn, request.psk
            )
            if not psk_file:
        ...
        if request.dhchap_key:
            ...
            if key_files_status != 0:
                ...
                return pb2.req_status(
                    status=key_files_status, error_message=key_file_errmsg
                )
        ...
        with omap_lock:
            try:
                if request.host_nqn == "*":  # Allow any host access to subsystem
                    ...
                    self._add_key_to_keyring(
                        "DH-HMAC-CHAP", dhchap_file, dhchap_key_name
                    )
                    self._add_key_to_keyring(
                        "DH-HMAC-CHAP controller",
                        dhchap_ctrlr_file,
                        dhchap_ctrlr_key_name,
                    )
                    ...
                    if psk_file:
                        ...
                        try:
                            rpc_keyring.keyring_file_remove_key(
                                self.spdk_rpc_client, psk_key_name
                            )
                        except Exception:
                            pass
                    if dhchap_file:
                        self.host_info.add_dhchap_host(
                            request.subsystem_nqn, request.host_nqn, request.dhchap_key
                        )
                ...
                if request.host_nqn == "*":
                    ...
                    self.remove_all_host_key_files(
                        request.subsystem_nqn, request.host_nqn
                    )
                    self.remove_all_host_keys_from_keyring(
                        request.subsystem_nqn, request.host_nqn
                    )
            ...
            if context:
                ...
                try:
                    ...
                    self.gateway_state.add_host(
                        request.subsystem_nqn, request.host_nqn, json_req
                    )
                except Exception as ex:
    ...
    def remove_host_from_state(self, subsystem_nqn, host_nqn, context):
        ...
        if context:
            assert (
                self.omap_lock.locked()
            ), "OMAP is unlocked when calling remove_host_from_state()"
        # Update gateway state
        try:
    ...
    def remove_host_safe(self, request, context):
        ...
        peer_msg = self.get_peer_message(context)
        all_host_failure_prefix = (
            f"Failure disabling open host access to {request.subsystem_nqn}"
        )
        ...
        if GatewayUtils.is_discovery_nqn(request.host_nqn):
            if request.host_nqn == "*":
                errmsg = (
                    f"{all_host_failure_prefix}: Can't use a discovery NQN as host's"
                )
            else:
                errmsg = f"{host_failure_prefix}: Can't use a discovery NQN as host's"
            self.logger.error(f"{errmsg}")
        ...
        with omap_lock:
            try:
                if request.host_nqn == "*":  # Disable allow any host access
                    ...
                    self.host_info.remove_psk_host(
                        request.subsystem_nqn, request.host_nqn
                    )
                    self.host_info.remove_dhchap_host(
                        request.subsystem_nqn, request.host_nqn
                    )
                    self.remove_all_host_key_files(
                        request.subsystem_nqn, request.host_nqn
                    ...
                    self.host_info.remove_host_nqn(
                        request.subsystem_nqn, request.host_nqn
                    )
            except Exception as ex:
                if request.host_nqn == "*":
                    ...
                    errmsg = f"{host_failure_prefix}:\n{ex}"
                self.logger.error(errmsg)
                self.remove_host_from_state(
                    request.subsystem_nqn, request.host_nqn, context
                )
                resp = self.parse_json_exeption(ex)
                status = errno.EINVAL
                if resp:
            ...
            return self.remove_host_from_state(
                request.subsystem_nqn, request.host_nqn, context
            )
    ...
    def change_host_key_safe(self, request, context):
        ...
        with omap_lock:
            host_psk = None
            if context:
                host_psk = self.host_info.get_host_psk_key(
                    request.subsystem_nqn, request.host_nqn
                )
            ...
            try:
                ...
                self._add_key_to_keyring(
                    "DH-HMAC-CHAP controller", dhchap_ctrlr_file, dhchap_ctrlr_key_name
                )
    ...
    def list_hosts_safe(self, request, context):
        ...
        peer_msg = self.get_peer_message(context)
        self.logger.info(
            f"Received request to list hosts for {request.subsystem}, context: {context}{peer_msg}"
        )
        try:
            ret = rpc_nvmf.nvmf_get_subsystems(
                self.spdk_rpc_client, nqn=request.subsystem
            ...
            if resp:
                status = resp["code"]
                errmsg = (
                    f"Failure listing hosts, can't get subsystems: {resp['message']}"
                )
    ...
    def list_connections_safe(self, request, context):
        ...
        self.logger.log(
            log_level,
            f"Received request to list connections for {request.subsystem}, context: {context}{peer_msg}",
        )
        ...
        if not request.subsystem:
            ...
            return pb2.connections_info(
                status=errno.EINVAL, error_message=errmsg, connections=[]
            )
        ...
        try:
            ...
            if resp:
                status = resp["code"]
                errmsg = (
                    f"Failure listing connections, can't get qpairs: {resp['message']}"
                )
            return pb2.connections_info(
                status=status, error_message=errmsg, connections=[]
            )
        ...
        try:
            subsys_ret = rpc_nvmf.nvmf_get_subsystems(
                self.spdk_rpc_client, nqn=request.subsystem
            )
            ...
            try:
                ...
                for qp in qpair_ret:
                    try:
                        ...
                        self.logger.exception(
                            f"Got exception while parsing qpair: {qp}"
                        )
                        pass
    ...
    def create_listener_safe(self, request, context):
        ...
        peer_msg = self.get_peer_message(context)
        self.logger.info(
            f"Received request to create {request.host_name}"
        ...
        with omap_lock:
            try:
                if request.host_name == self.host_name:
                    if (
                        ...
                        request.trsvcid,
                        False,
                    ) in self.subsystem_listeners[request.nqn] or (
                        adrfam,
                        traddr,
                        request.trsvcid,
                        True,
                    ) in self.subsystem_listeners[
                        request.nqn
                    ]:
                        self.logger.error(
                            f"{request.nqn} already listens on address {request.traddr}:{request.trsvcid}"
                        )
                        return pb2.req_status(
                            status=errno.EEXIST,
                    ...
                    ret = rpc_nvmf.nvmf_subsystem_add_listener(
                        self.spdk_rpc_client, **add_listener_args
                    )
                    self.logger.debug(f"create_listener: {ret}")
                    self.subsystem_listeners[request.nqn].add(
                        (adrfam, traddr, request.trsvcid, request.secure)
                    )
                else:
                    if context:
            ...
            if not ret:
                ...
                return pb2.req_status(
                    status=errno.EINVAL, error_message=create_listener_error_prefix
                )

            try:
                self.logger.debug(
                    f"create_listener nvmf_subsystem_listener_set_ana_state {request=} set inaccessible for all ana groups"
                )
                ...
                self.logger.debug(
                    f"create_listener nvmf_subsystem_listener_set_ana_state response {ret=}"
                )
                ...
                if self.ana_map[request.nqn]:
                    for x in range(self.subsys_max_ns[request.nqn]):
                        ana_grp = x + 1
                        if (
                            ana_grp in self.ana_map[request.nqn]
                            and self.ana_map[request.nqn][ana_grp]
                            == pb2.ana_state.OPTIMIZED
                        ):
                            _ana_state = "optimized"
                            self.logger.debug(
                                f"using ana_map: set listener on nqn : {request.nqn}  ana state : {_ana_state} for group : {ana_grp}"
                            )
                            ret = rpc_nvmf.nvmf_subsystem_listener_set_ana_state(
                                self.spdk_rpc_client,
                                nqn=request.nqn,
            ...
            if context:
                ...
                try:
                    ...
                    self.gateway_state.add_listener(
                        request.nqn,
                        request.host_name,
                        "TCP",
                        request.traddr,
                    ...
                    errmsg = (
                        f"Error persisting listener {request.traddr}:{request.trsvcid}"
                    )
                    self.logger.exception(errmsg)
                    errmsg = f"{errmsg}:\n{ex}"
                    return pb2.req_status(status=errno.EINVAL, error_message=errmsg)
    ...
    def remove_listener_from_state(self, nqn, host_name, traddr, port, context):
        ...
        if context:
            assert (
                self.omap_lock.locked()
            ), "OMAP is unlocked when calling remove_listener_from_state()"
        ...
        if host_name == "*":
            ...
            for key, val in state.items():
                ...
                try:
                    ...
                    if listener_nqn != nqn:
                        self.logger.warning(
                            f"Got subsystem {listener_nqn} instead of {nqn}, ignore"
                        )
                        continue
    ...
    def delete_listener_safe(self, request, context):
        ...
        host_msg = (
            "all hosts" if request.host_name == "*" else f"host {request.host_name}"
        )

        self.logger.info(
            f"Received request to delete TCP listener of {host_msg}"
        ...
        with omap_lock:
            try:
                if request.host_name == self.host_name or request.force:
                    ...
                    if request.nqn in self.subsystem_listeners:
                        if (
                            ...
                            request.trsvcid,
                            False,
                        ) in self.subsystem_listeners[request.nqn]:
                            self.subsystem_listeners[request.nqn].remove(
                                (adrfam, traddr, request.trsvcid, False)
                            )
                        if (
                            adrfam,
                            traddr,
                            ...
                            self.subsystem_listeners[request.nqn].remove(
                                (adrfam, traddr, request.trsvcid, True)
                            )
                else:
                ...
                if request.host_name == self.host_name:
                    ...
                    self.remove_listener_from_state(
                        request.nqn, request.host_name, traddr, request.trsvcid, context
                    )
                    resp = self.parse_json_exeption(ex)
                    status = errno.EINVAL
                    if resp:
            ...
            if not ret:
                ...
                return pb2.req_status(
                    status=errno.EINVAL, error_message=delete_listener_error_prefix
                )

            return self.remove_listener_from_state(
                request.nqn, request.host_name, traddr, request.trsvcid, context
            )
    ...
    def list_listeners_safe(self, request, context):
        ...
        peer_msg = self.get_peer_message(context)
        self.logger.info(
            f"Received request to list listeners for {request.subsystem}, context: {context}{peer_msg}"
        )
            ...
            for key, val in state.items():
                ...
                try:
                    ...
                    if nqn != request.subsystem:
                        self.logger.warning(
                            f"Got subsystem {nqn} instead of {request.subsystem}, ignore"
                        )
                        continue
                    secure = False
        ...
        return pb2.listeners_info(
            status=0, error_message=os.strerror(0), listeners=listeners
        )
    ...
    def list_subsystems_safe(self, request, context):
        ...
        if request.subsystem_nqn:
            self.logger.log(
                log_level,
                f"Received request to list subsystem {request.subsystem_nqn}, context: {context}{peer_msg}",
            )
        else:
            if request.serial_number:
                self.logger.log(
                    log_level,
                    f"Received request to list the subsystem with serial number {request.serial_number}, context: {context}{peer_msg}",
                )
            else:
                self.logger.log(
                    log_level,
                    f"Received request to list all subsystems, context: {context}{peer_msg}",
                )
        ...
        try:
            if request.subsystem_nqn:
                ret = rpc_nvmf.nvmf_get_subsystems(
                    self.spdk_rpc_client, nqn=request.subsystem_nqn
                )
            else:
            ...
            return pb2.subsystems_info_cli(
                status=status, error_message=errmsg, subsystems=[]
            )
        ...
        for s in ret:
            try:
                ...
                if s["subtype"] == "NVMe":
                    ...
                    s["enable_ha"] = True
                    s["has_dhchap_key"] = self.host_info.does_subsystem_have_dhchap_key(
                        s["nqn"]
                    )
                else:
                    s["namespace_count"] = 0
        ...
        return pb2.subsystems_info_cli(
            status=0, error_message=os.strerror(0), subsystems=subsystems
        )
    ...
    def get_subsystems_safe(self, request, context):
        ...
        peer_msg = self.get_peer_message(context)
        self.logger.debug(
            f"Received request to get subsystems, context: {context}{peer_msg}"
        )
        subsystems = []
        try:
    ...
    def change_subsystem_key_safe(self, request, context):
        ...
        peer_msg = self.get_peer_message(context)
        failure_prefix = (
            f"Failure changing DH-HMAC-CHAP key for subsystem {request.subsystem_nqn}"
        )
        self.logger.info(
            ...
            if context:
                ...
                if request.dhchap_key:
                    ...
                    for hostnqn in all_subsys_hosts:
                        ...
                        if not self.host_info.is_dhchap_host(
                            request.subsystem_nqn, hostnqn
                        ):
                ...
                assert (
                    subsys_entry
                ), f"Can't find entry for subsystem {request.subsystem_nqn}"
                try:
    ...
    def change_subsystem_key(self, request, context=None):
        """Change subsystem key."""
        return self.execute_grpc_function(
            self.change_subsystem_key_safe, request, context
        )
    ...
    def get_spdk_nvmf_log_flags_and_level_safe(self, request, context):
        ...
        peer_msg = self.get_peer_message(context)
        self.logger.info(
            f"Received request to get SPDK nvmf log flags and level{peer_msg}"
        )
        log_flags = []
        try:
            nvmf_log_flags = {
                key: value
                for key, value in rpc_log.log_get_flags(self.spdk_rpc_client).items()
            ...
            self.logger.debug(
                f"spdk log flags: {nvmf_log_flags}, "
    ...
    def get_spdk_nvmf_log_flags_and_level(self, request, context=None):
        return self.execute_grpc_function(
            self.get_spdk_nvmf_log_flags_and_level_safe, request, context
        )
    ...
    def set_spdk_nvmf_logs_safe(self, request, context):
        ...
        if request.HasField("log_level"):
            log_level = GatewayEnumUtils.get_key_from_value(
                pb2.LogLevel, request.log_level
            )
            if log_level == None:
        ...
        if request.HasField("print_level"):
            print_level = GatewayEnumUtils.get_key_from_value(
                pb2.LogLevel, request.print_level
            )
            if print_level == None:
        ...
        self.logger.info(
            f"Received request to set SPDK nvmf logs: log_level: {log_level}, print_level: {print_level}{peer_msg}"
        )

        try:
            nvmf_log_flags = [
                key
                for key in rpc_log.log_get_flags(self.spdk_rpc_client).keys()
                if key.startswith("nvmf")
            ]
            ret = [
                rpc_log.log_set_flag(self.spdk_rpc_client, flag=flag)
                for flag in nvmf_log_flags
            ]
            self.logger.debug(
                f"Set SPDK nvmf log flags {nvmf_log_flags} to TRUE: {ret}"
            )
            if log_level != None:
    ...
    def set_spdk_nvmf_logs(self, request, context=None):
        return self.execute_grpc_function(
            self.set_spdk_nvmf_logs_safe, request, context
        )
    ...
    def disable_spdk_nvmf_logs_safe(self, request, context):
        ...
        try:
            ...
            logs_level = [
                rpc_log.log_set_level(self.spdk_rpc_client, level="NOTICE"),
    ...
    def disable_spdk_nvmf_logs(self, request, context=None):
        return self.execute_grpc_function(
            self.disable_spdk_nvmf_logs_safe, request, context
        )
    ...
    def get_gateway_info_safe(self, request, context):
        ...
        if not cli_version_string:
            self.logger.warning(
                f"No CLI version specified, can't check version compatibility"
            )
        elif not cli_ver:
            self.logger.warning(
                f"Invalid CLI version {cli_version_string}, can't check version compatibility"
            )
        if ret.status == 0:
    ...
    def get_gateway_log_level(self, request, context=None):
        ...
        try:
            log_level = GatewayEnumUtils.get_key_from_value(
                pb2.GwLogLevel, self.logger.level
            )
        except Exception:
            self.logger.exception(
                f"Can't get string value for log level {self.logger.level}"
            )
            return pb2.gateway_log_level_info(
                status=errno.ENOKEY, error_message=f"Invalid gateway log level"
            )
        self.logger.info(
            f"Received request to get gateway's log level. Level is {log_level}{peer_msg}"
        )
        return pb2.gateway_log_level_info(
            status=0, error_message=os.strerror(0), log_level=log_level
        )
    ...
    def set_gateway_log_level(self, request, context=None):
        ...
        peer_msg = self.get_peer_message(context)
        log_level = GatewayEnumUtils.get_key_from_value(
            pb2.GwLogLevel, request.log_level
        )
        if log_level == None:
            errmsg = f"Unknown log level {request.log_level}"
            self.logger.error(f"{errmsg}")
        ...
        self.logger.info(
            f"Received request to set gateway's log level to {log_level}{peer_msg}"
        )
        ...
        try:
            ...
            self.logger.exception(
                f'Failure removing "{GatewayLogger.NVME_GATEWAY_LOG_LEVEL_FILE_PATH}"'
            )
        ...
        try:
            ...
            self.logger.exception(
                f'Failure writing log level to "{GatewayLogger.NVME_GATEWAY_LOG_LEVEL_FILE_PATH}"'
            )
