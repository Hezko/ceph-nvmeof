...
COLLECTION_ELAPSED_WARNING = (
    0.8  # Percentage of the refresh interval before a warning message is issued
)
...
def start_exporter(spdk_rpc_client, config, gateway_rpc, logger_to_use):
    ...
    if ssl:
        ...
        if os.path.exists(cert_filepath) and os.path.exists(key_filepath):
            httpd_ok = start_httpd(
                port=port, certfile=cert_filepath, keyfile=key_filepath
            )
        else:
            httpd_ok = False
            logger.error(
                "Unable to start prometheus exporter - missing cert/key file(s)"
            )
    else:
    ...
    if httpd_ok:
        logger.info(
            f"Prometheus exporter running in {mode} mode, listening on port {port}"
        )
...
class NVMeOFCollector:
    ...
    def __init__(self, spdk_rpc_client, config, gateway_rpc):
        ...
        self.interval = config.getint_with_default(
            "gateway", "prometheus_stats_inteval", 10
        )
        ...
        if self.bdev_pools:
            logger.info(
                f"Stats restricted to bdevs in the following pool(s): {','.join(self.bdev_pools)}"
            )
        else:
    ...
    def _get_subsystems(self):
        ...
        subsystems_info = self.gateway_rpc.get_subsystems(
            pb2.get_subsystems_req(), None
        )
    ...
    def _list_subsystems(self):
        ...
        if resp.status != 0:
            logger.error(
                f"Exporter failed to execute list_subsystems: {resp.error_message}"
            )
            return {}
    ...
    def _get_connection_map(self, subsystem_list):
        ...
        for subsys in subsystem_list:
            resp = self.gateway_rpc.list_connections(
                pb2.list_connections_req(subsystem=subsys.nqn)
            )
            if resp.status != 0:
                logger.error(
                    f"Exporter failed to fetch connection info for {subsys.nqn}: {resp.error_message}"
                )
                continue
    ...
    def collect(self):
        ...
        if elapsed > self.interval:
            ...
            logger.warning(
                f"Stats refresh of {elapsed:.2f}s is close to exceeding the interval {self.interval}s"
            )
        else:
        ...
        for bdev in self.bdev_info:
            ...
            try:
                ...
                logger.debug(
                    f"no rbd information present for bdev {bdev.get('name')}, skipping"
                )
                continue

            rbd_pool = rbd_info.get("pool_name")
            rbd_namespace = rbd_info.get(
                "namespace", ""
            )  # namespace is not currently present
        ...
        for bdev in self.bdev_io_stats.get("bdevs", []):
            ...
            if bdev_name not in bdev_lookup:
                logger.debug(
                    f"i/o stats for bdev {bdev_name} skipped. Either not an rbd bdev, or excluded by 'prometheus_bdev_pools'"
                )
                continue
            ...
            if tick_rate:
                bdev_read_seconds.add_metric(
                    [bdev_name], (bdev.get("read_latency_ticks") / tick_rate)
                )
                bdev_write_seconds.add_metric(
                    [bdev_name], (bdev.get("write_latency_ticks") / tick_rate)
                )
        ...
        for subsys in self.subsystems:
            ...
            subsystem_metadata.add_metric(
                [
                    nqn,
                    subsys.serial_number,
                    subsys.model_number,
                    subsys_is_open,
                    ha_enabled,
                    self.gw_metadata.group,
                ],
                1,
            )
            ...
            for ns in subsys.namespaces:
                subsystem_namespace_metadata.add_metric(
                    [nqn, str(ns.nsid), ns.bdev_name, str(ns.anagrpid)], 1
                )
        ...
        for addr in listener_map.keys():
            ...
            subsystem_listener_iface_info.add_metric(
                [device_name, nic.operstate, nic.duplex, nic.mac_address], 1
            )
            subsystem_listener_iface_speed_bytes.add_metric([device_name], nic.speed)

            for nqn in listener_map[addr]:
                subsystem_listener_iface_nqn_info.add_metric([device_name, nqn], 1)
