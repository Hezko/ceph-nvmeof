...
class Parser:
    ...
    def __init__(self):
        self.parser = ErrorCatchingArgumentParser(
            prog="python3 -m control.cli", description="CLI to manage NVMe gateways"
        )
        ...
        self.parser.add_argument(
            "--verbose", help="Run CLI in verbose mode", action="store_true"
        )
    ...
    def cmd(self, actions=[], aliases=[], hlp=None):
        ...
        def decorator(func):
            ...
            parser = self.subparsers.add_parser(
                func.__name__, description=helpstr, aliases=aliases, help=helpstr
            )
...
class GatewayClient:
    ...
    def version(self, args):
        ...
        if args.format == "text" or args.format == "plain":
            ...
            out_ver = json_format.MessageToJson(
                cli_ver,
                indent=4,
    ...
    def gw_get_info(self):
        ...
        if gw_info.status == 0:
            ...
            if gw_ver == None:
                ...
                gw_info.error_message = (
                    f'Can\'t parse gateway version "{gw_info.version}".'
                )
            elif gw_ver < base_ver:
                gw_info.status = errno.EINVAL
                gw_info.bool_status = False
                gw_info.error_message = (
                    f"Can't work with gateway version older than {BASE_GATEWAY_VERSION}"
                )
        return gw_info
    ...
    def gw_info(self, args):
        ...
        if args.format == "text" or args.format == "plain":
            if gw_info.status == 0:
                ...
                out_func(
                    f"Gateway's load balancing group: {gw_info.load_balancing_group}"
                )
                ...
                if gw_info.max_hosts_per_subsystem:
                    out_func(
                        f"Gateway's max hosts per subsystem: {gw_info.max_hosts_per_subsystem}"
                    )
    ...
    def gw_get_log_level(self, args):
        ...
        if args.format == "text" or args.format == "plain":
            if ret.status == 0:
                level = GatewayEnumUtils.get_key_from_value(
                    pb2.GwLogLevel, ret.log_level
                )
                out_func(f'Gateway log level is "{level}"')
            else:
                err_func(f"{ret.error_message}")
        elif args.format == "json" or args.format == "yaml":
            out_log_level = json_format.MessageToJson(
                ret,
                indent=4,
                including_default_value_fields=True,
                preserving_proto_field_name=True,
    ...
    gw_actions = []
    gw_actions.append(
        {"name": "version", "args": [], "help": "Display gateway's version"}
    )
    gw_actions.append(
        {"name": "info", "args": [], "help": "Display gateway's information"}
    )
    gw_actions.append(
        {"name": "get_log_level", "args": [], "help": "Get gateway's log level"}
    )
    gw_actions.append(
        {
            "name": "set_log_level",
    ...
    def gw(self, args):
        ...
        if not args.action:
            self.cli.parser.error(
                f"missing action for gw command (choose from {GatewayClient.gw_choices})"
            )
    ...
    def spdk_log_level_get(self, args):
        ...
        if args.format == "text" or args.format == "plain":
            if ret.status == 0:
                ...
                level = GatewayEnumUtils.get_key_from_value(
                    pb2.LogLevel, ret.log_print_level
                )
    ...
    def spdk_log_level_set(self, args):
        ...
        try:
            req = pb2.set_spdk_nvmf_logs_req(
                log_level=log_level, print_level=print_level
            )
        except ValueError as err:
            self.cli.parser.error(f"invalid log level {log_level}, error {err}")
    ...
    spdk_log_set_args = [
        argument(
            "--level",
            "-l",
            help="SPDK nvmf log level",
            required=False,
            type=str,
            choices=get_enum_keys_list(pb2.LogLevel),
        ),
        argument(
            "--print",
            "-p",
            help="SPDK nvmf log print level",
            required=False,
            type=str,
            choices=get_enum_keys_list(pb2.LogLevel),
        ),
    ...
    def spdk_log_level(self, args):
        ...
        if not args.action:
            self.cli.parser.error(
                f"missing action for spdk_log_level command (choose from {GatewayClient.spdk_log_choices})"
            )
    ...
    def subsystem_add(self, args):
        ...
        try:
            ...
            Exception
        ) as ex:  # In case of an old gateway the returned value wouldn't have the nqn field
            pass
        if not new_nqn:
    ...
    def subsystem_list(self, args):
        ...
        subsystems = None
        try:
            subsystems = self.stub.list_subsystems(
                pb2.list_subsystems_req(
                    subsystem_nqn=args.subsystem, serial_number=args.serial_number
                )
            )
        except Exception as ex:
            subsystems = pb2.subsystems_info_cli(
                status=errno.EINVAL, error_message=f"Failure listing subsystems:\n{ex}"
            )
            ...
            if subsystems.status == 0:
                ...
                for s in subsystems.subsystems:
                    if args.subsystem and args.subsystem != s.nqn:
                        err_func(
                            "Failure listing subsystem {args.subsystem}: Got subsystem {s.nqn} instead"
                        )
                        return errno.ENODEV
                    if args.serial_number and args.serial_number != s.serial_number:
                        err_func(
                            "Failure listing subsystem with serial number {args.serial_number}: Got serial number {s.serial_number} instead"
                        )
                        return errno.ENODEV
                ...
                if len(subsys_list) > 0:
                    ...
                    if args.subsystem:
                        ...
                        out_func(
                            f"No subsystem with serial number {args.serial_number}"
                        )
                    else:
    ...
    def subsystem_change_key(self, args):
        ...
        req = pb2.change_subsystem_key_req(
            subsystem_nqn=args.subsystem, dhchap_key=args.dhchap_key
        )
        try:
    ...
    subsys_add_args = [
        ...
        argument(
            "--dhchap-key", "-k", help="Subsystem DH-HMAC-CHAP key", required=False
        ),
    ]
    subsys_del_args = [
        argument("--subsystem", "-n", help="Subsystem NQN", required=True),
        argument(
            "--force",
            help="Delete subsytem's namespaces if any, then delete subsystem. If not set a subsystem deletion would fail in case it contains namespaces",
            action="store_true",
            required=False,
        ),
    ]
    ...
    subsystem_actions = []
    subsystem_actions.append(
        {"name": "add", "args": subsys_add_args, "help": "Create a subsystem"}
    )
    subsystem_actions.append(
        {"name": "del", "args": subsys_del_args, "help": "Delete a subsystem"}
    )
    subsystem_actions.append(
        {"name": "list", "args": subsys_list_args, "help": "List subsystems"}
    )
    subsystem_actions.append(
        {
            "name": "change_key",
    ...
    def subsystem(self, args):
        ...
        if not args.action:
            self.cli.parser.error(
                f"missing action for subsystem command (choose from {GatewayClient.subsystem_choices})"
            )
    ...
    def listener_add(self, args):
        ...
        if args.format == "text" or args.format == "plain":
            if ret.status == 0:
                out_func(
                    f"Adding {args.subsystem} listener at {traddr}:{args.trsvcid}: Successful"
                )
            else:
                err_func(f"{ret.error_message}")
        elif args.format == "json" or args.format == "yaml":
        ...
        if args.format == "text" or args.format == "plain":
            if ret.status == 0:
                host_msg = (
                    "for all hosts"
                    if args.host_name == "*"
                    else f"for host {args.host_name}"
                )
                out_func(
                    f"Deleting listener {traddr}:{args.trsvcid} from {args.subsystem} {host_msg}: Successful"
                )
            else:
                err_func(f"{ret.error_message}")
        elif args.format == "json" or args.format == "yaml":
    ...
    def listener_list(self, args):
        ...
        listeners_info = None
        try:
            listeners_info = self.stub.list_listeners(
                pb2.list_listeners_req(subsystem=args.subsystem)
            )
        except Exception as ex:
            listeners_info = pb2.listeners_info(
                status=errno.EINVAL,
        ...
        if args.format == "text" or args.format == "plain":
            if listeners_info.status == 0:
                ...
                for l in listeners_info.listeners:
                    adrfam = GatewayEnumUtils.get_key_from_value(
                        pb2.AddressFamily, l.adrfam
                    )
    ...
    listener_add_args = listener_common_args + [
        ...
        argument(
            "--secure", help="Use secure channel", action="store_true", required=False
        ),
    ]
    listener_del_args = listener_common_args + [
        ...
        argument(
            "--force",
            help="Delete listener even if there are active connections for the address, or the host name doesn't match",
            action="store_true",
            required=False,
        ),
    ]
    ...
    listener_actions = []
    listener_actions.append(
        {"name": "add", "args": listener_add_args, "help": "Create a listener"}
    )
    listener_actions.append(
        {"name": "del", "args": listener_del_args, "help": "Delete a listener"}
    )
    listener_actions.append(
        {"name": "list", "args": listener_list_args, "help": "List listeners"}
    )
    ...
    def listener(self, args):
        ...
        if not args.action:
            self.cli.parser.error(
                f"missing action for listener command (choose from {GatewayClient.listener_choices})"
            )
    ...
    def host_add(self, args):
        ...
        if args.psk:
            if len(args.host_nqn) > 1:
                self.cli.parser.error(
                    f"Can't have more than one host NQN when PSK keys are used"
                )

        if args.dhchap_key:
            if len(args.host_nqn) > 1:
                self.cli.parser.error(
                    f"Can't have more than one host NQN when DH-HMAC-CHAP keys are used"
                )
        ...
        for one_host_nqn in args.host_nqn:
            ...
            if one_host_nqn == "*" and args.dhchap_key:
                self.cli.parser.error(
                    f"DH-HMAC-CHAP key is only allowed for specific hosts"
                )

            req = pb2.add_host_req(
                subsystem_nqn=args.subsystem,
            ...
            try:
                ...
                ret = pb2.req_status(
                    status=errno.EINVAL, error_message=f"{errmsg}:\n{ex}"
                )
            ...
            if args.format == "text" or args.format == "plain":
                if ret.status == 0:
                    if one_host_nqn == "*":
                        out_func(
                            f"Allowing open host access to {args.subsystem}: Successful"
                        )
                    else:
                        out_func(
                            f"Adding host {one_host_nqn} to {args.subsystem}: Successful"
                        )
                else:
                    err_func(f"{ret.error_message}")
            elif args.format == "json" or args.format == "yaml":
    ...
    def host_del(self, args):
        ...
        out_func, err_func = self.get_output_functions(args)
        for one_host_nqn in args.host_nqn:
            req = pb2.remove_host_req(
                subsystem_nqn=args.subsystem, host_nqn=one_host_nqn
            )
            ...
            if args.format == "text" or args.format == "plain":
                if ret.status == 0:
                    if one_host_nqn == "*":
                        out_func(
                            f"Disabling open host access to {args.subsystem}: Successful"
                        )
                    else:
                        out_func(
                            f"Removing host {one_host_nqn} access from {args.subsystem}: Successful"
                        )
                else:
                    err_func(f"{ret.error_message}")
            elif args.format == "json" or args.format == "yaml":
    ...
    def host_change_key(self, args):
        ...
        if args.host_nqn == "*":
            self.cli.parser.error(
                f"Can't change keys for host NQN '*', please use a real NQN"
            )
        ...
        if args.format == "text" or args.format == "plain":
            if ret.status == 0:
                out_func(
                    f"Changing key for host {args.host_nqn} on subsystem {args.subsystem}: Successful"
                )
            else:
                err_func(f"{ret.error_message}")
        elif args.format == "json" or args.format == "yaml":
    ...
    def host_list(self, args):
        ...
        try:
            hosts_info = self.stub.list_hosts(
                pb2.list_hosts_req(subsystem=args.subsystem)
            )
        except Exception as ex:
            hosts_info = pb2.hosts_info(
                status=errno.EINVAL,
    ...
    host_actions = []
    host_actions.append(
        {"name": "add", "args": host_add_args, "help": "Add host access to a subsystem"}
    )
    host_actions.append(
        {
            "name": "del",
    ...
    host_actions.append(
        {"name": "list", "args": host_list_args, "help": "List subsystem's host access"}
    )
    host_actions.append(
        {
            "name": "change_key",
    ...
    def host(self, args):
        ...
        if not args.action:
            self.cli.parser.error(
                f"missing action for host command (choose from {GatewayClient.host_choices})"
            )
    ...
    def connection_list(self, args):
        ...
        try:
            connections_info = self.stub.list_connections(
                pb2.list_connections_req(subsystem=args.subsystem)
            )
        except Exception as ex:
            connections_info = pb2.connections_info(
                status=errno.EINVAL,
                error_message=f"Failure listing hosts:\n{ex}",
                connections=[],
            )
    ...
    def connection(self, args):
        ...
        if not args.action:
            self.cli.parser.error(
                f"missing action for connection command (choose from {GatewayClient.connection_choices})"
            )
    ...
    def ns_add(self, args):
        ...
        if args.rbd_create_image:
            if args.size == None:
                self.cli.parser.error(
                    "--size argument is mandatory for add command when RBD image creation is enabled"
                )
            ...
            if args.size != None:
                self.cli.parser.error(
                    "--size argument is not allowed for add command when RBD image creation is disabled"
                )

        req = pb2.namespace_add_req(
            rbd_pool_name=args.rbd_pool,
    ...
    def ns_del(self, args):
        ...
        try:
            ret = self.stub.namespace_delete(
                pb2.namespace_delete_req(subsystem_nqn=args.subsystem, nsid=args.nsid)
            )
        except Exception as ex:
            ret = pb2.req_status(
                status=errno.EINVAL, error_message=f"Failure deleting namespace:\n{ex}"
            )

        if args.format == "text" or args.format == "plain":
            if ret.status == 0:
                out_func(
                    f"Deleting namespace {args.nsid} from {args.subsystem}: Successful"
                )
            else:
                err_func(f"{ret.error_message}")
        elif args.format == "json" or args.format == "yaml":
    ...
    def ns_resize(self, args):
        ...
        try:
            ret = self.stub.namespace_resize(
                pb2.namespace_resize_req(
                    subsystem_nqn=args.subsystem, nsid=args.nsid, new_size=ns_size
                )
            )
        except Exception as ex:
            ret = pb2.req_status(
                status=errno.EINVAL, error_message=f"Failure resizing namespace:\n{ex}"
            )
        ...
        if args.format == "text" or args.format == "plain":
            if ret.status == 0:
                ...
                out_func(
                    f"Resizing namespace {args.nsid} in {args.subsystem} to {sz_str}: Successful"
                )
            else:
                err_func(f"{ret.error_message}")
        elif args.format == "json" or args.format == "yaml":
    ...
    def ns_list(self, args):
        ...
        try:
            namespaces_info = self.stub.list_namespaces(
                pb2.list_namespaces_req(
                    subsystem=args.subsystem, nsid=args.nsid, uuid=args.uuid
                )
            )
        except Exception as ex:
            namespaces_info = pb2.namespaces_info(
                status=errno.EINVAL, error_message=f"Failure listing namespaces:\n{ex}"
            )
            ...
            if namespaces_info.status == 0:
                ...
                for ns in namespaces_info.namespaces:
                    if args.nsid and args.nsid != ns.nsid:
                        err_func(
                            "Failure listing namespace {args.nsid}: Got namespace {ns.nsid} instead"
                        )
                        return errno.ENODEV
                ...
                if len(namespaces_list) > 0:
                    ...
                    if args.nsid:
                        out_func(
                            f"No namespace {args.nsid} in subsystem {args.subsystem}"
                        )
                    elif args.uuid:
                        out_func(
                            f"No namespace with UUID {args.uuid} in subsystem {args.subsystem}"
                        )
                    else:
    ...
    def ns_get_io_stats(self, args):
        ...
        try:
            get_stats_req = pb2.namespace_get_io_stats_req(
        ...
        if args.format == "text" or args.format == "plain":
            if ns_io_stats.status == 0:
                ...
                stats_list.append(
                    ["Read Latency Ticks", ns_io_stats.read_latency_ticks]
                )
                stats_list.append(
                    ["Max Read Latency Ticks", ns_io_stats.max_read_latency_ticks]
                )
                stats_list.append(
                    ["Min Read Latency Ticks", ns_io_stats.min_read_latency_ticks]
                )
                stats_list.append(
                    ["Write Latency Ticks", ns_io_stats.write_latency_ticks]
                )
                stats_list.append(
                    ["Max Write Latency Ticks", ns_io_stats.max_write_latency_ticks]
                )
                stats_list.append(
                    ["Min Write Latency Ticks", ns_io_stats.min_write_latency_ticks]
                )
                stats_list.append(
                    ["Unmap Latency Ticks", ns_io_stats.unmap_latency_ticks]
                )
                stats_list.append(
                    ["Max Unmap Latency Ticks", ns_io_stats.max_unmap_latency_ticks]
                )
                stats_list.append(
                    ["Min Unmap Latency Ticks", ns_io_stats.min_unmap_latency_ticks]
                )
                stats_list.append(
                    ["Copy Latency Ticks", ns_io_stats.copy_latency_ticks]
                )
                stats_list.append(
                    ["Max Copy Latency Ticks", ns_io_stats.max_copy_latency_ticks]
                )
                stats_list.append(
                    ["Min Copy Latency Ticks", ns_io_stats.min_copy_latency_ticks]
                )
                ...
                if args.format == "text":
                    ...
                    table_format = "plain"
                stats_out = tabulate(
                    stats_list, headers=["Stat", "Value"], tablefmt=table_format
                )
                out_func(
                    f"IO statistics for namespace {args.nsid} in {args.subsystem}, bdev {ns_io_stats.bdev_name}:\n{stats_out}"
                )
            else:
    ...
    def ns_change_load_balancing_group(self, args):
        ...
        if args.format == "text" or args.format == "plain":
            if ret.status == 0:
                out_func(
                    f"Changing load balancing group of namespace {args.nsid} in {args.subsystem} to {args.load_balancing_group}: Successful"
                )
            else:
                err_func(f"{ret.error_message}")
        elif args.format == "json" or args.format == "yaml":
    ...
    def ns_set_qos(self, args):
        ...
        if args.nsid <= 0:
            self.cli.parser.error("nsid value must be positive")
        if (
            args.rw_ios_per_second == None
            and args.rw_megabytes_per_second == None
            and args.r_megabytes_per_second == None
            and args.w_megabytes_per_second == None
        ):
        ...
        if args.format == "text" or args.format == "plain":
            if args.rw_ios_per_second and (args.rw_ios_per_second % 1000) != 0:
                ...
                err_func(
                    f"IOs per second {args.rw_ios_per_second} will be rounded up to {rounded_rate}"
                )
        ...
        if args.format == "text" or args.format == "plain":
            if ret.status == 0:
                out_func(
                    f"Setting QOS limits of namespace {args.nsid} in {args.subsystem}: Successful"
                )
            else:
                err_func(f"{ret.error_message}")
        elif args.format == "json" or args.format == "yaml":
    ...
    def ns_add_host(self, args):
        ...
        for one_host_nqn in args.host_nqn:
            try:
                add_host_req = pb2.namespace_add_host_req(
                    subsystem_nqn=args.subsystem, nsid=args.nsid, host_nqn=one_host_nqn
                )
            ...
            if args.format == "text" or args.format == "plain":
                if ret.status == 0:
                    out_func(
                        f"Adding host {one_host_nqn} to namespace {args.nsid} on {args.subsystem}: Successful"
                    )
                else:
                    err_func(f"{ret.error_message}")
            elif args.format == "json" or args.format == "yaml":
    ...
    def ns_del_host(self, args):
        ...
        for one_host_nqn in args.host_nqn:
            try:
                del_host_req = pb2.namespace_delete_host_req(
                    subsystem_nqn=args.subsystem, nsid=args.nsid, host_nqn=one_host_nqn
                )
            ...
            if args.format == "text" or args.format == "plain":
                if ret.status == 0:
                    out_func(
                        f"Deleting host {args.host_nqn} from namespace {args.nsid} on {args.subsystem}: Successful"
                    )
                else:
                    err_func(f"{ret.error_message}")
            elif args.format == "json" or args.format == "yaml":
    ...
    ns_actions = []
    ns_actions.append(
        {"name": "add", "args": ns_add_args_list, "help": "Create a namespace"}
    )
    ns_actions.append(
        {"name": "del", "args": ns_del_args_list, "help": "Delete a namespace"}
    )
    ns_actions.append(
        {"name": "resize", "args": ns_resize_args_list, "help": "Resize a namespace"}
    )
    ns_actions.append(
        {"name": "list", "args": ns_list_args_list, "help": "List namespaces"}
    )
    ns_actions.append(
        {
            "name": "get_io_stats",
    ...
    ns_actions.append(
        {
            "name": "change_load_balancing_group",
    ...
    def namespace(self, args):
        ...
        if not args.action:
            self.cli.parser.error(
                f"missing action for namespace command (choose from {GatewayClient.ns_choices})"
            )
...
def main_common(client, args):
    client.logger.setLevel(
        GatewayEnumUtils.get_value_from_key(pb2.GwLogLevel, args.log_level.lower())
    )
    ...
    client.connect(
        args, server_address, server_port, client_key, client_cert, server_cert
    )
